...
export function mergeSearchResults(prev: ISearchResults | null, next: ISearchResults): ISearchResults {
    if (!prev) return next;
    ...
    merged.sort(compareSearchResults);

    return {
        ...next,
        results: merged,
        count: Math.max(prev.count ?? 0, next.count ?? 0, merged.length),
    };
}
...
function normalizeHighlights(highlights: string[]): string[] {
    return Array.from(new Set((highlights ?? []).map((h) => h.trim().toLowerCase()).filter(Boolean)));
}
...
export const RoomSearchView = ({ term, scope, promise, className, onUpdate, inProgress, ref }: Props): JSX.Element => {
    ...
    if (results === null) {
        ...
        const orderedResults = [...(results.results ?? [])].sort(compareSearchResults);
